...
class Engine:
    ...
    async def __embedding_message_callback(self, source, message):
        ...
        if message is not None:
            ...
            deserialize_embedding_covariance = self.trainer.deserialize_embedding(message.embedding_covariance)
            ...
            await self.node_selection_strategy_selector.add_embedding(source, deserialize_embedding, deserialize_embedding_covariance, current_neighbors)
    ...
    async def generate_and_send_embeddings(self):
        if self.node_selection_strategy_enabled:
            if self.nss_selector == "distribution":
                ...
                embedding, embedding_cov = await self.node_selection_strategy_selector.get_embeddings(model=self.trainer.model.get_model_embeddings(), dataloader=self.trainer.model.get_dataset_embeddings().train_dataloader())
                self.trainer.model.remove_dataset_embeddings()
                self.trainer.model.remove_model_embeddings()
                logging.info(f"Mean embedding:\n{embedding}")
                logging.info(f"Covariance embedding:\n{embedding_cov}")
                serialized_embedding = self.trainer.serialize_embedding(embedding)
                serialized_embedding_cov = self.trainer.serialize_embedding(embedding_cov)
                message = self.cm.mm.generate_embedding_message(serialized_embedding, serialized_embedding_cov)
