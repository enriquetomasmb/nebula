...
def test_optimizer_configuration_cifar10():
    model = FedGPDCIFAR10ModelResNet8()
...
def test_forward_pass_inference_with_prototypes_cifar10():
    model = FedGPDCIFAR10ModelResNet8()
    create_random_prototypes(model, 10, model.embedding_dim)  # Assuming the final feature dimension is 512
    input_tensor = torch.rand(32, 3, 32, 32)
    output = model(input_tensor)
    assert output.ndim == 1 and output.shape[0] == 32, "Output tensor should be 1D with a single class index during inference"


def test_loss_combination_with_prototypes_cifar10():
    model = FedGPDCIFAR10ModelResNet8()
    create_random_prototypes(model, 10, model.embedding_dim)
...
def test_loss_combination_without_all_prototypes_cifar10():
    model = FedGPDCIFAR10ModelResNet8()
