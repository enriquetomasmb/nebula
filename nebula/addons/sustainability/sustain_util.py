...
def get_cup_benchmark(fileName='CPU_benchmark_v4.csv'):
    ...
    cpu_tdp_dict = dict(zip(cpu_tdp_df['cpuName'], cpu_tdp_df['TDP']))
    return cpu_tdp_dict
...
def get_world_map(jsonFilePath='ne_10m_admin_0_countries/global_energy_mix.json', shpFilePath='ne_10m_admin_0_countries/ne_10m_admin_0_countries.shp' ):
    current_dir = os.path.dirname(os.path.abspath(__file__))    
    with open(os.path.join(current_dir, jsonFilePath), 'r', encoding='utf-8') as file:
...
def get_carbon_intensity(longitude, latitude):
    ...
    carbon_intensity_data, world_map = get_world_map(jsonFilePath='ne_10m_admin_0_countries/global_energy_mix.json', shpFilePath='ne_10m_admin_0_countries/ne_10m_admin_0_countries.shp' )
